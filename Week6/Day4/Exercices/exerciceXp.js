...
colors.forEach((val, index) => {
    index >=3 && console.log(`${index}${colors[3]} choice is ${val}`);
    index < 3 && console.log(`${index}${colors[i]} choice is ${val}`);
});
...
let usersName = users.map((val) => val.firstName)
console.log(usersName);

// 2. Using the filter() method, create a new array, containing only the Full Stack Residents.
let fullStackUsers = users.filter(val => val.role == "Full Stack Resident")
console.log(fullStackUsers);

// 3. Bonus : Chain the filter method with a map method, to return an array containing only the lastName of the Full Stack Residents.
let bonus = users.filter(val => val.role == "Full Stack Resident").map((val) => val.firstName)
console.log(bonus);
...
let combine = epic.reduce((acc, val) => acc + " " + val)
console.log(combine);
...
let studentsPassed = students.filter(val => val.isPassed)
