...
def check_answers () :
    #initialisation of the scores and the wrong answers list
    number_correct_answers = 0
    number_incorrect_answers = 0
    list_wrong_answers = []

    print("\n ---- The quizz starts ----")
    for quizz in data : # we start to loop in the list
        user_answer = input(quizz["question"]) # we ask the question to the user
        # we check if the user is right or wrong, and we change the score consequently
        if user_answer.lower() == quizz["answer"].lower():
            number_correct_answers += 1
        else :
            number_incorrect_answers += 1
            # when the user is wrong, we copy all the question and answer and store it in the wrong answers list
...
def inform_user (correct, incorrect, wrong_answers) :
    ...
    # loop in the wrong answers list to print all the info in the list
    ...
    print("\n --------------------")
    # we start the game again if there is more than 3 wrong aswers
    if incorrect > 3 :
